...
algo_ls = ['epsilon-greedy-t1', 'ucb-t1', 'kl-ucb-t1',
...
with open(fname, "r") as f:
    ...
    if not (len_line_ls == number_lines):
        print("\n", "*"*10, "Mistake:number of lines in the output data file should be",
              number_lines, "but have ", len_line_ls, "*"*10, "\n")
        errorFlag = True
    ...
    for line in line_ls:
        ...
        set_main.add(line[0]+"--"+line[1]+"--"+line[2]+"--" +
    ...
    if not len(set_main) == number_lines:
        print("\n", "*"*10, "Mistake: You didn't print all the combinations. Need ",
              number_lines, "but printed ", len(set_main), "*"*10, "\n")
        errorFlag = True
    ...
    for i in range(10):
        ...
        cmd = "python", "bandit.py", "--instance", line[0].strip(), "--algorithm", line[1].strip(), "--randomSeed", line[2].strip(
        ), "--epsilon", line[3].strip(), "--scale", line[4].strip(), "--threshold", line[5].strip(), "--horizon", line[6].strip()
        ...
        if not rep_REG == orig_REG:
            print("\n", "*"*10, "Mistake: Unable to reproduce result for REG ",
                  line_str, " orignal="+orig_REG+" reproduced="+rep_REG, "\t", "*"*10, "\n")
            # print(line_str)
            errorFlag = True
        else:
            print("REG checked OK", end="\t")
        if not orig_HIGHS == rep_HIGH:
            print("\n", "*"*10, "Mistake: Unable to reproduce result for HIGHS ",
                  line_str, " orignal="+orig_HIGHS+" reproduced="+rep_HIGH, "\t", "*"*10, "\n")
            # print(line_str)
            errorFlag = True
        else:
